			      ...
			      u64 local_time, u64 remote_time)
{
	...
	MHI_LOG("Time response: seq:%lx local: %llu remote: %llu (ticks)\n",
				 ...
				 u64 local_time, u64 remote_time)
{
	...
	MHI_LOG("Time response: seq:%lx local: %llu remote: %llu (us)\n",
			       ...
			       char *buf)
{
	...
	if (ret) {
		MHI_ERR("Failed to request time, seq:%lx, ret:%d\n", seq, ret);
		return scnprintf(buf, PAGE_SIZE,
				 "Request failed or feature unsupported\n");
	}
